...
//
// 22/12/2017
//  Added the ability to send absence reasons to KAMAR Absence Manager via the app.
//  This ability is only available to caregivers to avoid students sending in false data.
// 
// 
//  
...
function loadMenuHeader(currentpage, studentsearch, teachersearch) {
	...
	if (loggedIn())
	{
		...
		} else if(g_loginlevel == 2 || g_loginlevel == 3){ // If logged in user has caregiver access, show the report absense.
			$('#studentreport').show();
		}
		else
		{
...
function unbindSaveAttendanceOnLeave() {
	// $(document).unbind('saveattendancestate'); // unbind() has been depricated
				...
				if (timetableyear != undefined && timetableyear != "" && teachercode != undefined && teachercode != "" )
				{
					...
					$(data).find('TeacherAbsLogResults').find('Weeks').find('Week').each(function(weekindex){
						...
						$(this).find('Days').find('Day').each(function(dayindex){
							...
							// 	}
							// }
							// console.log("newday:checklist: ",newday.checklist);
						});
						var newweek = new Object();
						newweek.days = days;
						newweek.weekstart = weekstart;
						// console.log("checklist: ", days);
...
// Allows students to give absence reason from app. Requires the login key, student data and the portal URL. this is only available for caregiver logins, not student logins.
...
function saveAttendanceValues(isfinished) {
	...
	for(var i in g_newattendanceentries) {
		attendanceentryarray.push(i + '|' + g_newattendanceentries[i] + '|' + (g_newattendancereasons[i] != undefined ? g_newattendancereasons[i] : '')); // Pushes the marked attendance into the array
	}
	...
	// console.log(attendanceentryarray);
}
...
function loadStudentAttendanceForPeriod(loginkey, selectedteachercode, selecteddate, selectedperiod, selectedweek, attendanceentrystring, isfinished) {
	...
	console.log(g_apisettings.settingsversion);
...
$('#register-pn-mobile').live('click',function(event, data) {
	...
	$.post('https://api.school.kiwi/1.0/mobile/register/register_pn', {pnid: g_pnid, type: 2, mobile: $('#register-pn-mobile').val()}, function(data) {
        if(data.status) {
